...
for system in histogram_dictionary.keys():
    ...
    for function in functions:
        ...
        if function == "no_function":
            histogram_selection_dictionary[system][function] = {"S":[], "M":[], "T":[], "L":[]}
            
            for histogram in histogram_dictionary[system][function]["time"]:
                key = histogram.GetName().split("_")
                
                for selection in key[1:-1]:
                    if selection[1:] not in histogram_selection_dictionary[system][function][selection[0]]:
                       histogram_selection_dictionary[system][function][selection[0]].append(selection[1:])
...
for system in histogram_dictionary.keys():
    ...
    for function in functions:
        ...
        if function == "no_function":
            layer_selections = histogram_selection_dictionary[system][function]["L"]
